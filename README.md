...
of a ficus benjamini. The main function is `get_bezier_df()` which
...
leaf). The exact dimensions of these coordinates generated by random
numbers in the a certain range (see the definition of
`get_bezier_df()`).

## Illustration of the randomness

In order to show the variations of the `get_bezier_df()` (if parameters
...
dfb <- expand_grid(
  ...
  transpose() %>%
  map_dfr(~get_bezier_df(gen_benjamini_points(.x$x, .x$y)), .id = "leaf") %>%
  unite(i, i, leaf)
